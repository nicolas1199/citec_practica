...
export class AreasDocumentosSeeder {
    async run() {
        ...
        if (!fs.existsSync(archivoAreasDocumentosPath)) {
            throw new Error(
                `Archivo no encontrado: ${archivoAreasDocumentosPath}`,
            );
        }
        ...
        if (areaDocumentosExistentes > 0) {
            console.log(
                'Los codigos de area ya están cargadas en la base de datos.',
            );
            return;
        }

        const archivoAreasDocumentos = fs.readFileSync(
            archivoAreasDocumentosPath,
            'utf-8',
        );
