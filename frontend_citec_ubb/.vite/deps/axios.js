...
function buildURL(url, params, options) {
    ...
    if (serializeFn) {
        ...
        serializedParams = utils_default.isURLSearchParams(params)
            ? params.toString()
            : new AxiosURLSearchParams_default(params, options).toString(
                  _encode,
              );
    }
    if (serializedParams) {
        ...
        Object.assign(
            {
                visitor: function (value, key, path, helpers) {
                    if (
                        platform_default.isNode &&
                        utils_default.isBuffer(value)
                    ) {
        ...
        function deleteHeader(_header) {
            ...
            if (_header) {
                ...
                if (
                    key &&
                    (!matcher ||
                        matchHeaderValue(self2, self2[key], key, matcher))
                ) {
                    delete self2[key];
                    deleted = true;
                }
        ...
        return new Promise(function dispatchXhrRequest(resolve, reject) {
            ...
            request.ontimeout = function handleTimeout() {
                let timeoutErrorMessage = _config.timeout
                    ...
                    : 'timeout exceeded';
                const transitional2 =
                    _config.transitional || transitional_default;
            ...
            if (
                protocol &&
                platform_default.protocols.indexOf(protocol) === -1
            ) {
                reject(
                    new AxiosError_default(
    ...
    _request(configOrUrl, config) {
        ...
        if (transitional2 !== void 0) {
            validator_default.assertOptions(
                transitional2,
                {
                    silentJSONParsing: validators2.transitional(
                        validators2.boolean,
                    ),
                    forcedJSONParsing: validators2.transitional(
                        validators2.boolean,
                    ),
                    clarifyTimeoutError: validators2.transitional(
                        validators2.boolean,
                    ),
                },
                false,
        ...
        let contextHeaders =
            headers &&
            utils_default.merge(headers.common, headers[config.method]);
        headers &&
            utils_default.forEach(
