import { require_react_dom } from './chunk-TNTPHDQH.js';
...
function convertRoutesToDataRoutes(
    ...
    return routes.map((route, index) => {
        ...
        if (isIndexRoute(route)) {
            ...
            let pathOrLayoutRoute = _extends(
                {},
                route,
...
function createRouter(init) {
    ...
    async function handleLoaders(
        ...
        if (redirect3) {
            if (redirect3.idx >= matchesToLoad.length) {
                let fetcherKey =
                    revalidatingFetchers[redirect3.idx - matchesToLoad.length]
                        .key;
    ...
    async function startRedirectNavigation(request, redirect3, _temp2) {
        ...
        let redirectHistoryAction =
            replace2 === true ||
            redirect3.response.headers.has('X-Remix-Replace')
                ? Action.Replace
                : Action.Push;
    ...
    async function callLoadersAndMaybeResolveData(
        ...
        await Promise.all([
            ...
            resolveDeferredResults(
                ...
                fetcherResults,
                fetchersToLoad.map((f) =>
                    f.controller ? f.controller.signal : null,
                ),
                true,
            ),
        ]);
    ...
    function handleDiscoverRouteError(pathname, discoverResult) {
        return {
            boundaryId: findNearestBoundary(discoverResult.partialMatches).route
                .id,
            error: getInternalRouterError(400, {
                type: 'route-discovery',
                pathname,
                message:
                    discoverResult.error != null &&
                    'message' in discoverResult.error
                ...
                _extends(
                    {
                        currentUrl,
                        currentParams:
                            state.matches[state.matches.length - 1].params,
                        nextUrl,
...
async function convertHandlerResultToDataResult(handlerResult) {
    ...
    if (isDeferredData(result)) {
        ...
        return {
            ...
            deferredData: result,
            statusCode:
                (_result$init3 = result.init) == null
                    ? void 0
                    : _result$init3.status,
            headers:
                ((_result$init4 = result.init) == null
                    ? void 0
                    : _result$init4.headers) &&
    ...
    if (isDataWithResponseInit(result)) {
        ...
        return {
            ...
            data: result.data,
            statusCode:
                (_result$init5 = result.init) == null
                    ? void 0
                    : _result$init5.status,
            headers:
...
function mapRouteProperties(route) {
    let updates = {
        ...
        hasErrorBoundary:
            route.ErrorBoundary != null || route.errorElement != null,
    };
    if (route.Component) {
        if (true) {
            if (route.element) {
...
function createMemoryRouter(routes, opts) {
    return createRouter({
        ...
        unstable_dataStrategy:
            opts == null ? void 0 : opts.unstable_dataStrategy,
                ...
                React2.createElement(
                    ...
                    React2.createElement(
                        ...
                        React2.createElement(
                            ...
                            state.initialized ||
                                router.future.v7_partialHydration
